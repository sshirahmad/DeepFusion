...
class LinearDecayWithWarmup(_LRSchedulerStep):
    def __init__(
        self, optimizer, total_steps, last_step=-1
    ):
        self._total_steps = total_steps
    ...
    def _get_lr_per_group(self, base_lr):
        ...
        step = self.last_step
        if step < self._total_steps // 3:
            learning_rate = base_lr
        elif step < self._total_steps * 2 // 3:
            learning_rate = base_lr / 10.
        else:
            learning_rate = base_lr / 100.

        return learning_rate


class PolynomialDecayWithWarmup(_LRSchedulerStep):
    def __init__(
        self, optimizer, total_steps, warmup_learning_rate, warmup_steps, last_step=-1
    ):
        if total_steps < warmup_steps:
    ...
    def _get_lr_per_group(self, base_lr):
        ...
        learning_rate = base_lr * (1 - (step / self._total_steps)) ** 0.9
...
class RangeTestLR(_LRSchedulerStep):
    def __init__(
        self, optimizer, total_steps, start_lr, end_lr, last_step=-1
    ):
        self._total_steps = total_steps
    ...
    def _get_lr_per_group(self, base_lr):
        ...
        learning_rate = self._start_lr + (step / self._total_steps) * (self._end_lr - self._start_lr)
