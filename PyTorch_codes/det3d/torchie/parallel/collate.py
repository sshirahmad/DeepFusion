...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    for example in batch_list:
        if type(example) is list:
            ...
            for k, v in example.items():
                example_merged[k].append(v)
    batch_size = len(example_merged['image'])
    ret = {}
    ...
    for key, elems in example_merged.items():
        if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
            ...
            ret[key] = res
        elif key in ["gt_boxes_cls", "yolo_map1", "yolo_map2", "yolo_map3",
                     "target_boxes_grid1", "target_boxes_grid2", "target_boxes_grid3",
                     "classifier_map1", "classifier_map2", "classifier_map3",
                     "obj_mask1", "obj_mask2", "obj_mask3", "noobj_mask1", "noobj_mask2",
                     "noobj_mask3", "grid_map1", "grid_map2", "grid_map3", "grid_map4", "image",
                     "obj_mask", "noobj_mask"]:
