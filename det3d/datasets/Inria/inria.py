...
class InriaDataset(PointCloudDataset):
    # RGB channels
    NumPointFeatures = 3
    ...
    def ground_truth_annotations(self):
        if "gt_boxes" not in self._inria_infos[0]:
            return None
        gt_annos = []
        for info in self._inria_infos:
    ...
    def evaluation(self, detections, iou_thres=0.5, img_dim=416, output_dir=None, val_mode=False):
        ...
        for i, gt in enumerate(gt_annos):
            if detections[gt["token"]]["boxes"] is not None:
                j += detections[gt["token"]]["boxes"].size(0)
                ...
                metrics, boxes, scores = yolo_utils.match_boxes(detections[gt["token"]], target_boxes,
