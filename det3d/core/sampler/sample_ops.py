...
class DataBaseSamplerV2:
            ...
            if sampled_num > 0:
                ...
                if len(sampled_cls) > 0:
                    if len(sampled_cls) == 1:
                        sampled_gt_box = sampled_cls[0]["box3d_radar"][np.newaxis, ...]
                    else:
                        sampled_gt_box = np.stack(
                            [s["box3d_radar"] for s in sampled_cls], axis=0
                        )
    ...
    def sample_class_v2(self, name, num, gt_boxes):
        ...
        sp_boxes = np.stack([i["box3d_radar"] for i in sampled], axis=0)
        ...
        for i in range(num_gt, num_gt + num_sampled):
            if coll_mat[i].any():
                ...
                if self._enable_global_rot:
                    sampled[i - num_gt]["box3d_radar"][:2] = boxes[i, :2]
                    sampled[i - num_gt]["box3d_radar"][-1] = boxes[i, -1]
    ...
    def sample_group(self, name, num, gt_boxes, gt_group_ids):
        ...
        for num in group_num:
            if coll_mat[idx : idx + num].any():
                ...
                for i in range(num):
                    if self._enable_global_rot:
                        sampled[idx - num_gt + i]["box3d_radar"][:2] = boxes[
                            idx + i, :2
                        ]
                        sampled[idx - num_gt + i]["box3d_radar"][-1] = boxes[
                            idx + i, -1
                        ]
