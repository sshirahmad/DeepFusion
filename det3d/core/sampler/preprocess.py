...
def ImgAug(img, corner_bbox, labels, data_aug_seq):
    """img augumentation
    Args:
    ...
    """
    bboxes = []
    for bbox, name in zip(corner_bbox, labels):
        xmin = bbox[0]
        ymin = bbox[1]
        xmax = bbox[2]
        ymax = bbox[3]
        bboxes.append(ia.BoundingBox(x1=xmin, y1=ymin, x2=xmax, y2=ymax, label=name))
    ...
    image_aug, bbs_aug = data_aug_seq(image=img, bounding_boxes=bbs)
    bbs_aug = bbs_aug.remove_out_of_image().clip_out_of_image()

    bboxes = []
    for bbox in bbs_aug.bounding_boxes:
        bboxes.append(np.array([bbox.x1, bbox.y1, bbox.x2, bbox.y2]))

    bboxes = np.array(bboxes)

    return bboxes, image_aug
