...
def _create_learning_rate_scheduler(optimizer, learning_rate_config, total_step):
    ...
    elif learning_rate_type == "cosine_warmup":
        lr_scheduler = ls.CosineDecayWithWarmup(
            optimizer, total_step, config.warmup_learning_rate, config.warmup_steps, last_step=-1
        )
    elif learning_rate_type == "linear_warmup":
        lr_scheduler = ls.LinearDecayWithWarmup(
            optimizer, total_step, last_step=-1
        )
    elif learning_rate_type == "polynomial_warmup":
        lr_scheduler = ls.PolynomialDecayWithWarmup(
            optimizer, total_step, config.warmup_learning_rate, config.warmup_steps, last_step=-1
        )
    elif learning_rate_type == "range_test":
        lr_scheduler = ls.RangeTestLR(
            optimizer, total_step, config.start_lr, config.end_lr, last_step=-1
        )
